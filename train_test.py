...
def main(rank, world_size):
    ...
    disentangle = model_test.disentangle(device=rank).to(device)
    ...
    optimizer = optim.Adam(list(disentangle.module.parameters()), lr=lr)
    ...
    for epoch in range(1,epochs+1): 
        ...
        for (batch_idx, train_tensors) in enumerate(train_loader):
            z,p,mfcc,rms,_ = train_tensors   

            z = z.to(device)[:,0,:,:]
            p = p.to(device)
            mfcc = mfcc.to(device)
            rms = rms.to(device)
            l,_ = disentangle(z,p,mfcc, rms)
            
            # if epoch < training_params["encoder_epochs"]:
            #     loss = l["p_predict"] + l["m_predict"] + l["r_predict"]
            # else:
            #     loss = l["t_predict"] + l["pm_ce_loss"] + l ["pr_ce_loss"] + l["mr_ce_loss"]
            
            loss = l["t_predict"] + l["pm_ce_loss"] + l ["pr_ce_loss"] + l["mr_ce_loss"] + l["p_predict"] + l["m_predict"] + l["r_predict"]
            ...
            loss_total_ce += ( l["pm_ce_loss"] + l ["pr_ce_loss"] + l["mr_ce_loss"]).item()
        ...
        if (epoch % v_frequency) == 0:
            if rank == 0:
                ...
                for v_data in valid_loader:
                    ...
                    l,predict = disentangle(z,p,mfcc, rms)
                    ...
                    v_loss_total_ce += (l["p_ce_loss"] + l["m_ce_loss"] + l["r_ce_loss"] + l["pm_ce_loss"] + l ["pr_ce_loss"] + l["mr_ce_loss"]).item()
                    v_loss_total = v_loss_total_pitch + v_loss_total_mfcc + v_loss_total_rms + v_loss_total_r + v_loss_total_ce
                    ...
                    writer.add_image(f"Pitch/Ground Truth-{i}", make_pitch_img(samples, p[0]), dataformats='HWC')
                    ...
                    writer.add_image(f"RMS/Ground Truth-{i}", make_rms_img(rms), dataformats='HWC')
                ...
                z,p,mfcc,rms,z_prime = next(iter(valid_loader))
                ...
                for i in range(4):
                    z_predict = disentangle.module.get_new_sample(z_codes[0].unsqueeze(0), rms=curves[i].unsqueeze(0).unsqueeze(0).to(device))
                    z_predict = model.quantizer.from_codes(z_predict)[0]
                    out = model.decode(z_predict)
